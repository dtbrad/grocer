...
describe Product do
  ...
  describe "knows about its purchase history" do
    before :each do
      ...
      @basket_one = @user.baskets.create(transaction_date: '2016-01-01')

      @line_item_one = @product_one.line_items.create(basket_id: @basket_one.id, user_id: @user.id, quantity: 1, price_cents: 900, total_cents: 900)
      @line_item_two = @product_two.line_items.create(basket_id: @basket_one.id, user_id: @user.id, quantity: 20, price_cents: 500, total_cents: 10000)

      @basket_two = @user.baskets.create(transaction_date: '2016-01-02')
      @line_item_two = @product_one.line_items.create(basket_id: @basket_two.id, user_id: @user.id, quantity: 5, price_cents: 300, total_cents: 1500)

      @basket_three = @user_two.baskets.create(transaction_date: '2016-01-01')
      @line_item_three = @product_one.line_items.create(basket_id: @basket_three.id, user_id: @user_two.id,quantity: 1, price_cents: 600, total_cents: 600)
      @line_item_four = @product_two.line_items.create(basket_id: @basket_three.id, user_id: @user_two.id, quantity: 3, price_cents: 400, total_cents: 1200)
