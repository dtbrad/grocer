...
describe Basket do
  ...
  context 'It knows about itself' do
    ...
    let(:graph_config1) { GraphConfig.new(start_date: '2017-01-14 08:59:54 -0800', end_date: '2017-04-14 08:59:54 -0800',
                                     unit: 'month') }
    let(:graph_config2) { GraphConfig.new(start_date: '2017-01-14 08:59:54 -0800', end_date: '2017-02-14 08:59:54 -0800',
                                     unit: 'week') }
    let(:graph_config3) { GraphConfig.new(start_date: '2017-01-14 08:59:54 -0800', end_date: '2017-01-20 08:59:54 -0800',
                                     unit: 'day') }

    let(:baskets1) { Basket.from_graph(graph_config1) }
    let(:baskets2) { Basket.from_graph(graph_config2) }
    let(:baskets3) { Basket.from_graph(graph_config3) }
    ...
    it 'groups properly' do
      grouped_baskets1 = baskets1.group_baskets(graph_config1)
      grouped_baskets2 = baskets2.group_baskets(graph_config2)
      grouped_baskets3 = baskets2.group_baskets(graph_config3)

      expect(grouped_baskets1).to eq([[Date.parse('2017-01-01'), 10000], [Date.parse('2017-02-01'),500],
                             [Date.parse('2017-03-01'), 200], [Date.parse('2017-04-01'), 500]])

      expect(grouped_baskets2).to eq([[Date.parse('2017-01-08'), 3000], [Date.parse('2017-01-15'), 7000],
                             [Date.parse('2017-01-22'), 0], [Date.parse('2017-01-29'), 0],
                             [Date.parse('2017-02-05'), 0], [Date.parse('2017-02-12'), 500]])

      expect(grouped_baskets3).to eq([[Date.parse('2017-01-14'), 3000], [Date.parse('2017-01-15'), 0],
